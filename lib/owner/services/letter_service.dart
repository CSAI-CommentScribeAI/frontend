...
class LetterService {
  ...
  // 편지 불러오기 API
  Future<List<Map<String, dynamic>>> getLetter() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse is Map && jsonResponse['data'] is List) {
          final List<dynamic> letters = jsonResponse['data'];
          print('JSON 데이터: $letters');

          for (var letter in letters) {
            if (letter is Map<String, dynamic>) {
              letterInstance.add(letter);
            }
          }

          print('조회 성공 $letterInstance');
